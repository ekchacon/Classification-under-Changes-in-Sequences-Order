      ...
      for LAYER in range(rLayer,(MAXLAYER+1),1):
        ...
        for x in range(rUnits, (MAXUNITS+1),1): # ten*2 files per kind of layer
          ...
          cbks = [tf.keras.callbacks.LearningRateScheduler(lambda epoch: (1e-3)/((epoch+1)**(1/2))), #(1e-3)/(epoch+1) #0.001
                  ...
                  tf.keras.callbacks.CSVLogger('/home/est1/Edgar/tensorflowPractice/tfLSTMSupervisedModelEvaluation/mnist_c/dynamic/log_'+formReadPixel+'_'+str(row)+'x'+str(column)+'_L'+str(LAYER)+'_U'+str(UNITS)+'.csv', append=True, separator=',')
                  ]#(write_graph=True)] 
          ...
          gs.best_estimator_.model.save_weights('/home/est1/Edgar/tensorflowPractice/tfLSTMSupervisedModelEvaluation/mnist_c/dynamic/log_'+formReadPixel+'_'+str(row)+'x'+str(column)+'_L'+str(LAYER)+'_U'+str(UNITS)+'.h5')
          #time.sleep(60)
          ...
          path = '/home/est1/Edgar/tensorflowPractice/tfLSTMSupervisedModelEvaluation/mnist_c/dynamic/log_'+formReadPixel+'_'+str(row)+'x'+str(column)+'_L'+str(LAYER)+'_U'+str(UNITS)+'.csv'
          ...
          f = open('/home/est1/Edgar/tensorflowPractice/tfLSTMSupervisedModelEvaluation/mnist_c/dynamic/log_'+formReadPixel+'_'+str(row)+'x'+str(column)+'.csv','a')
...
#rowColumnInputShape(X_train,X_test,y_train,y_test,rRow=392,rFlag=1,rLayer=3,rUnits=9,formReadPixel ='H')
#Horizontal
...
#plt.imshow(X_train[10])
#rowColumnInputShape(X_train,X_test,y_train,y_test,rRow=784,rFlag=1,rLayer=3,rUnits=9,formReadPixel ='V')
#Vertical
